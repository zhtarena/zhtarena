		...
		<div class="head_content" id="headBar">
			...
			<div id="finder-head-8adea0" class="mainHead headBox">
				<div class="finder-head">
					...
					<div class="span-2 pointer" title="公文状态">
						公文状态
					</div>
...
相关操作
		...
		<div class="main_content" id="main"
			style="height: 460px; visibility: visible; opacity: 1;">
							...
							<div style="cursor: pointer;" class="row-line">
								...
								<div class="span-1 span-select">
									<input type="checkbox" value="4" name="member_id[]" class="sel"
										tags="null" />
									<input type="hidden" value="${user.id }" name="id" class="sel"
										tags="null" />
								</div>
								...
								<div class="cell span-4" key="ctime">
									<fmt:formatDate pattern="yyyy年MM月dd日  HH:mm:ss" value="${doc.ctime }"
												var="dateline"></fmt:formatDate>${ dateline}
												
								</div>
								<div class="cell span-2" key="dstatus">
								
								<c:if test="${doc.dstatus==0}">新建</c:if>
								<c:if test="${doc.dstatus==1}">待审批</c:if>
								<c:if test="${doc.dstatus==2}">审批通过</c:if>
								<c:if test="${doc.dstatus==3}">审批失败</c:if>
								...
								<c:if test="${!empty doc.flow.appendix}">
								<a href="oa/pages/flows/download.jsp?filename=${doc.appendix }"> <img border="none"
								...
								<c:set var="appendix" value="${doc.appendix }"></c:set>
   ...
   <c:out value="${splitapp}"></c:out>
								...
								<c:if test="${docurl=='myselfstatus1'}">		
									<a href='javascript:new opwindowhistory(${doc.id });void(0);'>审批历史</a>						
									</c:if>
									<c:if test="${docurl=='myselfstatus2'}">
									<a href='javascript:new opwindowhistory(${doc.id });void(0);'>审批历史</a>
									</c:if>
									<c:if test="${docurl=='myselfstatus3'}">
									<a href='javascript:new opwindowhistory(${doc.id });void(0);'>审批历史</a>
									</c:if>
									...
									<c:if test="${docurl=='myself'}">
									&nbsp;
									</c:if>
									<c:if test="${docurl=='myselfstatus0'}">
									<a href="javascript:new opwindow(${doc.id });void(0);">提交</a>&nbsp;&nbsp;
									<a href="doc.do?method=findDocById&id=${doc.id }">修改</a>&nbsp;&nbsp;
									<a href="doc.do?method=delDoc&id=${doc.id }">删除</a>
									</c:if>
									<c:if test="${docurl=='myselfstatus1'}">		
									&nbsp;							
									</c:if>
