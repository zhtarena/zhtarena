...
<a href="">模块管理</a>
» 
  <a class="" href="module.do?method=findModules">查看模块</a>
  » 
  <span class="">修改模块</span>
</div></div>
      <div class="main_content" id="main" style="height: 400px; width: 1352px; visibility: visible; opacity: 1;">
<form onsubmit="javascript:return j_submit();" id="moduleform" name="moduleform" action="module.do?method=doUpModule" method="post">
<div class="tableform">
    <div class="division">
      <table cellspacing="0" cellpadding="0" border="0">
        ...
        <tr>
          ...
          <input onblur="checkname(this);" type="hidden" name="id" value="${requestScope.module.id}"/>
          <input type="text" name="name" value="${requestScope.module.name }"/><span id="j_name"></span>
            </td>
        </tr>
       <c:if test="${!empty requestScope.fmodules}"><!-- 存在一级模块 -->
       <c:if test="${!empty requestScope.pmodules}"><!-- 存在二级模块 -->
       		 <tr>
          <th>一级模块:</th>
          <td><select onchange="changefirstModule()" selected="8" vtype="select" class="_x_ipt select" labelcolumn="cat_name" valuecolumn="cat_id" value="8" style="font-size:12px;" name="pmodule" id="firstModule" type="select">
          <option value="0" label="----无----">----无----</option>
<c:forEach items="${requestScope.fmodules}" var="fmodule">
 	<c:if test="${fmodule.id==requestScope.pmodule.pmodule.id}">
 	<option selected="selected" value="${fmodule.id}" label="${fmodule.name}">${fmodule.name}</option>
 	</c:if>
 	<c:if test="${fmodule.id!=requestScope.pmodule.pmodule.id}">
	<option value="${fmodule.id}" label="${fmodule.name}">${fmodule.name}</option>
        ...
        <tr>
          ...
          <option value="0" label="----无----">----无----</option>
<c:forEach items="${requestScope.pmodules}" var="module2">
 	<c:if test="${module2.id==requestScope.pmodule.id}">
 	<option selected="selected" value="${module2.id}" label="${module2.name}">${module2.name}</option>
 	</c:if>
 	<c:if test="${module2.id!=requestScope.pmodule.id}">
	<option value="${module2.id}" label="${module2.name}">${module2.name}</option>
 	 </c:if>
</c:forEach>
</select>
        ...
        <c:if test="${empty requestScope.pmodules}"><!-- 当前模块为二级模块 -->
        <tr>
          <th>一级模块:</th>
          <td><select selected="8" vtype="select" class="_x_ipt select" labelcolumn="cat_name" valuecolumn="cat_id" value="8" style="font-size:12px;" name="pmodule" id="firstModule" type="select">
          <option value="0" label="----无----">----无----</option>
<c:forEach items="${requestScope.fmodules}" var="fmodule">
 	<c:if test="${fmodule.id==requestScope.pmodule.id}">
 	<option selected="selected" value="${fmodule.id}" label="${fmodule.name}">${fmodule.name}</option>
 	</c:if>
 	<c:if test="${fmodule.id!=requestScope.pmodule.id}">
	<option value="${fmodule.id}" label="${fmodule.name}">${fmodule.name}</option>
          ...
          <c:if test="${!empty requestScope.cmodules}">
        <tr>
          <th>删除子分类:</th>
          <td>
          <c:forEach items="${requestScope.cmodules}" var="cmodule">
          <span><input type="checkbox" name="cmodules" value="${cmodule.id }" />${cmodule.name }</span>
          </c:forEach>
          <span>请把要删除的子分类勾选</span>
          </td>
        </tr>
               </c:if>
          <tr>
          <th>模块的链接路径:</th>
          <td>
          <input  type="text" name="url" value="${requestScope.module.url }"/>
            </td>
        </tr>
         <tr>
          <th>模块显示的target:</th>
           <td>
          <input onblur="checktarget(this);" type="text" name="target" value="${requestScope.module.target }"/><span id="j_target"></span>
            </td>
        </tr>   
          <tr>
          <th>模块的类型:</th>
          <td>
          <input onblur="checktype(this);" type="text" name="type" value="${requestScope.module.type }"/>
        ...
        <tr>
          <th>模块排序:</th>
           <td>
          <input onblur="checksort(this);" type="text" name="sort" value="${requestScope.module.sort }"/><span id="j_sort"></span>
            </td>
        </tr>   
      <tr>
  <th>模块图标：</th>
...
<input type="submit" value="修改" name="修改"></form>
</div>
